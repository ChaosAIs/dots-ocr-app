...
class Gemma3OCRConverter:
    ...
    def detect_primary_language(self, text: str) -> str:
        """Ask Gemma3 to classify the primary language of the document text.

        The model infers the language itself instead of us hard-coding a small
        set of languages. On any error or ambiguous reply, this returns an empty
        string so that callers can fall back or ignore the signal.
        """
        cleaned = (text or "").strip()
        if not cleaned:
            return ""

        # Limit the amount of text we send to the model for efficiency.
        if len(cleaned) > 4000:
            cleaned = cleaned[:4000]

        if requests is None:  # pragma: no cover - hit only when requests missing
            self.logger.error(
                "requests library not installed - cannot call Ollama/Gemma3 for language detection",
            )
            return ""

        prompt = (
            "You will be given markdown text from a document.\n"  # type: ignore[str-bytes-safe]
            "Determine the primary human language that is most used in the content "  # type: ignore[str-bytes-safe]
            "(for example Chinese, English, French, Japanese, Spanish, etc.).\n"  # type: ignore[str-bytes-safe]
            "Respond with exactly one word naming that language, no explanation.\n\n"  # type: ignore[str-bytes-safe]
            "Document content:\n"  # type: ignore[str-bytes-safe]
            f"{cleaned}\n"
        )
        ...
        try:
            self.logger.info(
                "Sending language detection request to Gemma3 via Ollama: model=%s, endpoint=%s",
                self.model_name,
                self.generate_endpoint,
            )
            ...
            self.logger.error("Error calling Gemma3 for language detection: %s", exc)
            return ""

        if response.status_code != 200:
            self.logger.error(
                "Gemma3/Ollama language detection returned non-200 status: %s - %s",
                response.status_code,
                response.text,
            )
            return ""
        ...
        try:
            ...
            self.logger.error("Failed to parse Gemma3/Ollama language JSON response: %s", exc)
            return ""

        result = (data.get("response") or "").strip()
        if not result:
            self.logger.warning("Gemma3/Ollama language detection returned empty response")
            return ""
